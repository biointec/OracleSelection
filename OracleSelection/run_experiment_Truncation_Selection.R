...
tp.change<-c("best", "random", "PEVmean", "CDmean", "tails", "trainSel")

# Verify that the TP changes in the arguments are acceptable
if (!all(tp.change %in% c("best", "worst", "random", "nochange", "PEVmean", "CDmean", "tails", "trainSel")) )
...
parents.sel.intensity = 50 #pas aan -------------------------------------------------------------------------------
...
for (change in tp.change) {
  ...
  experiment.sub.results <- mclapply(X = iters.per.core, FUN = function(iter.set) {
    ...
    sapply(X = reps, FUN = function(r) {
      ...
      for (breeding.cycle in seq(n.cycles)) {
        ...
        qtl.marker.LD.i<-NA #calculation of QTL LD has been removed
      ...
      # Estimate marker effects
        predictions.out <- try(make.predictions(pheno.train = TP.phenos.i,
                                            ...
                                            geno.pred = candidate.marker.genos.i))   #this can be also be changed to TBV or phenotypic values
        #to use TBV or phenotypic values first run candidate.values.i before predictions.out 
       ...
       #candidate.values.i$mean.pheno.values
        parent.selections.i <- select.population(value.mat =  candidate.values.i$geno.values, #predictions.out$GEBV, 
        ...
        if (change != "nochange") {
          if (change == "trainSel"){
            data.train<-MakeTrainSelData(M=candidate.marker.genos.i,
                                         K=A.sc)
            
            train<-TrainSel(Data= data.train,
                            Candidates =list(1:dim(candidate.marker.genos.i)[1]),
                            setsizes = tp.update.increment,
                            settypes = "UOS" ,
                            Stat= NULL, control=TrainSelControl())
            
            selid<- train$BestSol_int
            TP.addition.list <-list(TP.addition.lines = rownames(candidate.marker.genos.i)[selid])
          }
  ...
  filename <- file.path(save.dir, paste("simulation_results_Baseline_GV_", pop.makeup, "_", change, "_", 
