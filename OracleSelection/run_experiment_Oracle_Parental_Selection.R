  ...
  experiment.sub.results <- mclapply(X = iters.per.core, FUN = function(iter.set) {
    ...
    sapply(X = reps, FUN = function(r) {
      ...
      G_G0<-genotypic.value(genome = hv.genome, haploid.genos = TP.haploids.i) %>%
        mean()
      G_G0_top<-sort(genotypic.value(genome = hv.genome, haploid.genos = TP.haploids.i),decreasing=TRUE) %>%
        mean()
      ...
      for (breeding.cycle in seq(n.cycles)) {
        ...
        haploid.mat <- do.call("rbind", candidate.haploid.i)
        source("R/QTL_max.R")
        ...
        selectie<-Oracle.Selection(haploids=haploid.mat,hv.genome,N=parents.sel.intensity) 
        
        
        parent.lines.list <- list(p1 = selectie, 
                                  p2 = selectie)
