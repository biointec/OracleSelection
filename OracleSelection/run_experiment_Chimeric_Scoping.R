...
S.value=c(round(seq(0.1,1,length.out = 30),1),rep(1,20))
...
for (change in tp.change) {
  ...
  experiment.sub.results <- mclapply(X = iters.per.core, FUN = function(iter.set) {
    ...
    hv.genome <- Genome$genome[[rep.iter]]
      ...
      for (breeding.cycle in seq(n.cycles)) {
        ...
        s.value=S.value[breeding.cycle]
        ...
        source("R/make_crossing_block_ChimericScoping.R")
        }else{
        ...
        while(selection.crit){
          ...
          parent.selections.i <- select.population(value.mat = predictions.out$GEBV, 
                                                   sel.intensity = round(parents.sel.intensity*SR), 
                                                   selection = "best") 
          
          preselected.geno<-candidate.marker.genos.i[parent.selections.i$lines.sel,]
          prefixed<-0
          for (i in seq(dim(preselected.geno)[2])){
            if (all(preselected.geno[,i]==-1) | all(preselected.geno[,i]==1)){
              prefixed<-prefixed+1
            }
          }
          
          if (prefixed>nfixed & SR<10){
            SR<-SR+0.01
          }else{
            selection.crit<-F
          }
        ...
        simulation.results[[cycle.name]] <- 
          list(geno.summary.stats = list(candidate.af = candidate.af.i,
               ...
               fixation = list(parents=parents.fixed, preselection= prefixed),
  ...
  filename <- file.path(save.dir, paste("Scoping_Maxvar_s",s.value ,"_",n.iterations,"rep_","_",date, ".RData", sep = "") )                      
